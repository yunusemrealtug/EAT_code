...
def process_frame(modnet, frame, background_color):
    ...
    face = frame * face_matte + (1 - face_matte) * 255
...
def process_video(input_video_path, output_video_path, modnet, background_color):
    ...
    while cap.isOpened():
        ...
        final_frame = process_frame(modnet, frame, background_color)
...
def process_frames(modnet):
    if not os.path.exists('./demo/imgs_segmented/'):
        os.makedirs('./demo/imgs_segmented/')
    
    if not os.path.exists('./demo/masks/'):
        os.makedirs('./demo/masks/')

    background_color = (255, 255, 255)

    allimgs_cropped = glob.glob('./demo/imgs_cropped/*.jpg')
    for img_path in tqdm(allimgs_cropped):
        base_name = os.path.splitext(os.path.basename(img_path))[0]
        img_name_png = f"{base_name}_mask.png"
        img_name = os.path.basename(img_path)
        output_path = os.path.join('./demo/imgs_segmented/', img_name)
        mask_path = os.path.join('./demo/masks/', img_name_png)
        cropped_copy_path = os.path.join('./demo/masks/', os.path.basename(img_path).replace('.jpg', '.png'))
    ...
    print("Masks saved to './demo/masks/'")
...
def prepare_test_data(img_path, audio_path, opt, emotype, use_otherimg=True):
    ...
    if use_otherimg:
        source_latent = np.load(img_path.replace('segmented', 'latent')[:-4]+'.npy', allow_pickle=True)
    else:
...
def process_video_with_custom_background(input_video_path, output_video_path, modnet, background_np):
    cap = cv2.VideoCapture(input_video_path)
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = cap.get(cv2.CAP_PROP_FPS)
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        final_frame = process_frame(modnet, frame, background_np)
        final_frame = cv2.cvtColor(final_frame, cv2.COLOR_RGB2BGR)
        out.write(final_frame)
...
def test(ckpt, emotype, save_dir=" "):
    ...
    modnet = load_modnet("MODNet/modnet_photographic_portrait_matting.ckpt")
    ...
    process_frames(modnet)

    script_path = 'lama/bin/predict.py'
    model_path = os.path.join(os.getcwd(), 'lama/big-lama')
    ...
    for ind in tqdm(range(len(all_wavs2))):
        for img_path in tqdm(allimg_cropped):
            ...
            video_path2 = os.path.join(log_dir, "temp2", f_name)
            imageio.mimsave(video_path, predictions_gen, fps=25.0)         
            save_video = os.path.join(log_dir, f_name)
            
            #background_color = (255, 255, 255)
            background_filename = os.path.basename(img_path)  # Get the filename from img_path
            background_filename = os.path.splitext(background_filename)[0]  # Remove the extension
            background_filename += '_mask.png'  # Add the new extension
            background_path = os.path.join('demo/backgrounds', background_filename)
            background_np = load_background(background_path) 

            # Videoyu işle
            process_video_with_custom_background(video_path, video_path2, modnet, background_np)
            cmd = r'ffmpeg -loglevel error -y -i "%s" -i "%s" -vcodec copy -shortest "%s"' % (video_path2, audio_path, save_video)
            os.system(cmd)
